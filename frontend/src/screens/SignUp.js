                    ...
                    <div className="mx-auto px-4 md:px-10 lg:px-20 xl:px-48 mt-4">
                        ...
                        <div className="flex flex-col items-center mt-12">
                            ...
                            <button className=" border-2 mt-5 p-3 w-2/3 sm:w-2/4 md:w-1/3 lg:w-1/5 text-white rounded-3xl bg-white"  type="button">
                                <img className="btn-auth-img" src={googleButton} alt='google sign up'/>
                            </button>

                            <button onClick={signupHandler} type="button" className=" mt-3 p-3 w-2/3 sm:w-2/4 md:w-1/3 lg:w-1/5 text-white rounded-3xl hover:bg-blue-900 bg-blue-700 disabled:bg-blue-500" disabled={email.length < 1 || password.length < 8 || password !== confirmPassword || name.length < 1}>Sign Up</button>

                            <p className="mt-4 mb-10">Already have an account? <Link to="/login" className="text-blue-700" >Sign in</Link></p>
                        </div>
                    </div>
                )}
            </Fragment>
