...
const UpdateProfile = () => {
    ...
    // Email validation: Can improve this with more complex regex if needed
    const validateEmail = (email) => /\S+@\S+\.\S+/.test(email);

    // Password validation: At least 8 characters, with uppercase, lowercase, number, special character
    ...
    const changeNameHandler = () => {
        if (validateName(name)) {
            ...
            alert('Name should only contain letters and numbers.');
        }
    };
    ...
    const changePasswordHandler = () => {
        if (newPassword === confirmPassword) {
            if (validatePassword(newPassword)) {
                ...
                alert('Password must be at least 8 characters long, include uppercase letters, lowercase letters, numbers, and special characters.');
            }
        } else {
